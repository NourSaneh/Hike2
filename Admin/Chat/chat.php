...
// $result = mysqli_query($con, "SELECT u.id, u.username, r.role 
//                               FROM users u 
//                               JOIN userrole r ON u.roleid = r.id 
//                               WHERE u.id != $user_id AND roleid=3 ");
$result = $con->query("SELECT u.id, u.username, r.role 
FROM users u 
JOIN userrole r ON u.roleid = r.id 
WHERE u.id != $user_id AND u.roleid= 3 ");
